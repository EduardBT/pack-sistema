...
$where = "WHERE rol = 'destinatario'";
$search_query = "";
if (isset($_POST["enviar-nombre"])) {
    $valor = $_POST['campo'];
    if (!empty($valor)) {
        $search_query = "AND (dni LIKE '%$valor%' OR nombre LIKE '%$valor%' OR apellidos LIKE '%$valor%' OR departamento LIKE '%$valor%')";
        $where .= $search_query;
    }
}
...
<div class="card shadow full-screen" style="padding:2%;margin-left:1px; background-color: #fff;">
    ...
    <div class="card-header" style="padding:0 0 10px 0">
        <div class="d-flex justify-content-between align-items-center">
            <h3 class="mb-0"> <i class="bi bi-person-lines-fill"></i>DESTINATARIOS</h3>
            <a href="nuevo-destinatario.php" class="btn btn-success"><i class="bi bi-plus"
                    style="margin-right:5px"></i>Agregar</a>
        </div>
    </div>
    <form method="POST">
        <div class="input-group mt-3">
            <input class="form-control" type="search" id="busqueda" name="campo" placeholder="Buscar..."
                onkeyup="buscar()" aria-label="Buscar" style="border-radius:0">
