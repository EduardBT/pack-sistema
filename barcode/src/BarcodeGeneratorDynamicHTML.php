...
{
    private const WIDTH_PRECISION = 12;
    ...
    public function getBarcode($barcode, $type, string $foregroundColor = 'black')
    {
        ...
        $html = '<div style="font-size:0;position:relative;width:200%;height:200%">' . PHP_EOL;

        $positionHorizontal = 0;
        /** @var BarcodeBar $bar */
        foreach ($barcodeData->getBars() as $bar) {
            $barWidth = $bar->getWidth() / $barcodeData->getWidth() * 100;
            $barHeight = round(($bar->getHeight() / $barcodeData->getHeight() * 100), 6);

            if ($bar->isBar() && $barWidth > 0) {
                $positionVertical = round(($bar->getPositionVertical() / $barcodeData->getHeight() * 100), 6);
